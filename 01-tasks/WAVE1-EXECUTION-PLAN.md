...
## 目标
执行当前未完成开发任务：BACKEND-014、SHOP-FE-012、SHOP-FE-013，并按依赖顺序完成 QA 验证。

## 依赖分析
- SHOP-FE-013 依赖 BACKEND-014 完成并通过 QA，且 API 文档同步状态为 synced。
- SHOP-FE-012 与 BACKEND-014 无直接依赖，可并行推进。

## 波次编排

### Wave 1（并行开发）
**范围**：BACKEND-014 + SHOP-FE-012
...
2) SHOP-FE-012（商城前端）
- QA 阻塞点：接口返回与任务契约不一致，需兼容映射。
- 目标：在 `getOrderById` 或订单详情页取值处做兼容映射；保留已完成的“去支付”跳转与 i18n 修复。

**调度方式**：
- 依次执行 requeue → dispatch（同一时刻只发一条 dispatch 指令）。
- 两个任务可并行执行（由 worker pool 自行并行）。
