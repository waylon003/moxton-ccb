...
- 若决策已经被现有链路规则覆盖（如 `runtime/orchestration`、`env/service`、`qa_evidence`、`code/contract/ui`、`qa_reject`、`review_reject`、`stale_run`、`offline_drift`），Team Lead 必须直接执行，不得再向用户询问。
- 只有当阻塞不属于既有分类，且规则源/执行源/知识源都无法裁决时，才允许升级询问用户。
...
## Execution Engine
- Team Lead 会话：Claude Code（工作目录固定为 `E:\moxton-ccb`）
- 执行层：当前业务主链默认使用 headless `codex exec`
- 兼容说明：`gemini` 仅保留为单次兼容覆盖，不作为默认执行引擎
- WezTerm 角色：只承担 Team Lead 宿主、`route-notifier` 最后一跳唤醒和 Rich 看板挂载；不再作为日常 worker 主执行现场
- 通知链：`route-monitor.ps1` 写事件，`route-notifier.ps1` 独立通过 WezTerm `send-text` 唤醒 Team Lead
- 不再依赖 Agent Teams / `notify-sentinel`
- 如需关闭直接唤醒，可设置 `CCB_ROUTE_MONITOR_NOTIFY=0`
- 当前业务主链默认使用无审批模式的 Codex worker：`-a never --sandbox danger-full-access`
## 路由唤醒规则（新增，强制）
...
- 收到 `[ROUTE] ... next=status/check_routes` 后：
  ...
  2. 若 `status` 显示多任务并行、阻塞根因不清、notifier 投递异常或 headless 运行态需要对照，再查看右侧 Rich 看板
  3. 再根据需要执行 `check_routes`
  4. 根据 route 内容做决策（如 `dispatch-qa` / `requeue` / `archive`）
...
- 回调提醒由 `route-monitor` 写事件，再由 `route-notifier` 唤醒 Team Lead。
...
- 禁止把 `wezterm cli get-text` 当作主链监控手段；主链默认只看 `status` / Rich / route 提醒。仅人工调试时才允许短时查看 `get-text`
...
6. **Execution 分支**：
  ...
  - 若检测到任务处于执行态但对应运行实例已离线（如 `OFFLINE-DRIFT` / 旧 pid 漂移 / run_dir 残留），必须先：
  ...
  - **dispatch 后由控制器自动确保后台能力：**
    - `route-monitor.ps1`（任务锁与 route 收口）
    - `route-notifier.ps1`（MCP route 唤醒）
    - `rich-monitor`（只读看板，若已启用则一并判活）
  - 监控防死循环规则（必须）：
     - 主链默认只看 `status`、Rich 和 route 提醒，不以 `get-text` 作为常规观察入口。
     - 同一 task 在收到提醒后若连续 3 轮 `status` 无新 route/runtime 变化，必须停止轮询并转入恢复分支。
     - 只有人工调试时才允许短时查看 `get-text`，且最多 3 轮；禁止出现“get-text -> status”无限重复。
...
- 判定顺序固定如下：
  ...
  2. 若 worker 在线，但 `status` 出现 `[STALE-RUN>...m]` 或 `next=开发长时间无新 route / QA 长时间无新 route`
     - 结论：这是卡住
  ...
  5. 若任务已收到 `fail` 回传
     - 结论：这是终态失败，不再等待更多 route；主任务按 `blocked` 同级决策处理
     - 处理：先执行 `status`，读取 `body` / `note` / `routeUpdate.bodyPreview`；若是 QA fail，默认回开发（必要时先恢复环境）；若是开发 fail，优先 `recover -RecoverAction restart-task`，再决定 `requeue` 与重新 `dispatch`
- 禁止在未执行 `status` 的情况下，仅凭终端暂时没输出就认定 worker 卡住。
...
7. **QA**：
  ...
  - 前端 QA 浏览器验收默认顺序：`Playwright smoke -> agent-browser 真实交互验收 -> 结构化证据补充（截图 / console / network）`
  - QA 通过只表示验收通过，不自动提交代码
  - 若 Team Lead 复审驳回，先 `requeue -TargetState waiting_qa`，不要把驳回原因直接发到旧 QA 运行上下文
  - 复审驳回后的重新验收默认使用 fresh QA context；只有“补证据/补报告格式”才考虑复用原 QA 运行上下文
