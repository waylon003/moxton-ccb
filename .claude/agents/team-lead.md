...
- 通知：Agent Teams notify-sentinel（审批/回调提醒）
- notify-sentinel 不会随 dispatch 自动启动；每个新会话必须手动创建并让其阅读 `E:\\moxton-ccb\\.claude\\agents\\notify-sentinel.md`
- **创建后必须发送启动指令让其进入轮询**：`@notify-sentinel 读取 E:\\moxton-ccb\\.claude\\agents\\notify-sentinel.md 并立刻进入循环；只要检测到新增 route/approval/local approval 事件，第一动作必须先给我发一条 teammate 消息，再继续轮询；禁止只更新 ready.json 或只在你自己的窗口总结。`
- Claude Code UI/手机端的审批与回调提醒依赖 notify-sentinel（非 WezTerm 注入）
- 若设置 `CCB_ROUTE_MONITOR_NOTIFY=0`（禁用 route-monitor 唤醒），必须创建 notify-sentinel，否则不会收到任何回传/审批提醒。
- Codex CLI 本地审批提示不会写入 approval-requests.json；由 `dispatch/dispatch-qa` 自动拉起的 `pane-approval-watcher.ps1` 负责捕获，并写入 `config/local-approval-state.json` / `config/local-approval-events.jsonl` / `config/teamlead-alerts.jsonl`。
- notify-sentinel 启动后必须输出一次 [WATCH-READY]；未出现则重建 teammate
- dispatch/dispatch-qa 前必须有 notify-sentinel ready 标记；若无，先执行 `powershell -NoProfile -ExecutionPolicy Bypass -File "E:\moxton-ccb\scripts\teamlead-control.ps1" -Action notify-ready`。注意：`notify-ready` 只补 ready 标记，不能替代 notify-sentinel 真正进入轮询
- 默认启用严格模式：派遣前必须创建 notify-sentinel；若确需关闭，设置 `TEAMLEAD_STRICT_NOTIFY_SENTINEL=0`。
- 高风险审批不得询问用户；由 Team Lead 直接 approve/deny，默认拒绝，除非任务文档明确允许

## 路由唤醒规则（新增，强制）

- `route-monitor` 默认只负责写 `config/teamlead-alerts.jsonl`；notify-sentinel 才是负责转发唤醒的队友。
...
- 审批/回调提醒由 Agent Teams notify-sentinel 负责；本地 pane 审批由 per-worker `pane-approval-watcher.ps1` 捕获后写入事件文件，再由 notify-sentinel 转发给 Team Lead。
...
- 禁止使用 Task(...) / Backgrounded agent 进行派遣（会绕过主链路）；允许 Agent Teams notify-sentinel 仅做通知
...
6. **Execution 分支**：
  ...
  - **dispatch 后由控制器自动确保后台能力：**
    ...
    - notify-sentinel（Agent Teams 通知队友，审批/回调提醒）
  - 审批优先级硬规则（必须）：
  ...
  - 监控防死循环规则（必须）：
   ...
   - 若回调来自 `*-qa` 且 `status=success`，必须先做证据门禁再决定是否接受：
     ...
     - 禁止仅凭“已验证/已通过”口头描述放行
  - notify-sentinel 检测到审批提醒后：先判断是文件型审批还是本地 pane 审批，再走对应命令：
