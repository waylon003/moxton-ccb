...
- `doc-updater` 有两种模式：
  - `backend_qa`：开发期实时同步，优先避免前端读取过期 API 文档
  - `archive_move` / `round_complete`：归档期一致性复核，若文档已正确则回传 `success` 且 `result=noop`
- `repo-committer` 的语义约束：
  - `artifact_cleanup_required` 才是阻塞，必须 `status=blocked`
  - `no_changes_to_commit` 不是阻塞，必须回传 `status=success` 且 `result=noop`
  - Team Lead 收到 `success + result=noop` 时，应按“仓库已达目标状态”处理，不得盲目重派
