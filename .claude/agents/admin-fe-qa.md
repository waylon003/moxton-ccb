...
- 所有截图、日志、JSON、txt 都必须写到这个目录，禁止写到 `E:\moxton-lotadmin\05-verification\...`
...
6. 自动化测试优先级：
   ...
   - 推荐 `agent-browser` 工作流：
     ...
     - `agent-browser get url --session admin-fe-qa-<TASK-ID>`（确认当前页面）
     - `agent-browser wait --load networkidle --session admin-fe-qa-<TASK-ID>`（页面较慢时先等）
...
7. 验证要点：
   ...
   - UI 细节：表格对齐、按钮尺寸、弹窗层级、禁用态/错误态是否符合任务要求（必须有截图证据）
   - API 交互：请求参数和响应处理是否正确（必须采集关键接口响应状态，包含 4xx/5xx 检查）
   - 边界情况：空数据、错误响应、权限不足
...
- 所有 evidence 的磁盘真实文件必须位于 `E:\moxton-ccb\05-verification\<TASK-ID>\`；禁止写到 `E:\moxton-lotadmin\05-verification\...`。
