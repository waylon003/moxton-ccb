...
verified_against: [BACKEND-016, ADMIN-FE-007, ADMIN-FE-006, ADMIN-FE-005]
---
...
联调和 QA 默认先通过 `GET /health`、`GET /version` 确认后端实例已就绪；该运行基线已于 2026-03-24 文档同步时再次复核通过。
...
| `/orders/admin/:id/ship` | PUT | 发货 |
| `/orders/admin/:id/deliver` | PUT | 确认收货 |
| `/orders/admin/:id/shipping-info` | PATCH | 补充物流信息 |
| `/orders/admin/:id/history` | GET | 操作历史 |
| `/offline-orders/admin` | GET | 咨询订单列表 |
| `/offline-orders/admin/:id` | GET/PUT | 咨询订单详情 / 更新状态与备注 |
