...
def default_ccb_worker(task: TaskInfo) -> str:
    mapping = {
        "SHOP-FE": "shop-fe-dev",
        "ADMIN-FE": "admin-fe-dev",
        "BACKEND": "backend-dev",
    }
    return mapping.get(task.role_code, "worker")


def ccb_ask(worker: str, message: str) -> bool:
    """Send message to a worker via `ask` command."""
    try:
...
def ccb_pend(worker: str, timeout: int = 300) -> str | None:
    """Wait for worker response via `pend` command."""
    try:
        result = subprocess.run(
            ["pend", worker],
            capture_output=True,
...
def ccb_ping(worker: str) -> bool:
    """Check worker status via `ping` command."""
    try:
        result = subprocess.run(
            ["ping", worker],
            capture_output=True,
...
def dispatch_ccb_task(root: Path, task: TaskInfo, worker: str, note: str) -> tuple[str, bool]:
    """Dispatch task to worker using CCB ask command."""
...
2. 请阅读上述任务文档并实现所有要求
3. 完成后报告修改的文件列表和测试证据
4. 如有跨角色依赖，使用 [ROUTE] 信封格式向 Team Lead 发送消息{note_section}
...
"""
    ...
    if success:
        log_dir = ensure_ccb_runs_dir(root)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dispatch-ccb",
        help="Dispatch one task to CCB worker via ask command",
    )
    parser.add_argument(
        "--poll-ccb",
        action="store_true",
        help="Poll one CCB worker response via pend command",
    )
    parser.add_argument(
        "--ccb-worker",
        default="",
        help="Optional worker name for --dispatch-ccb/--poll-ccb",
    )
    parser.add_argument(
        "--ccb-note",
        default="",
        help="Optional dispatch note for CCB request",
    )
    parser.add_argument(
        "--ccb-timeout",
        type=int,
        default=300,
        help="Timeout seconds for --poll-ccb (pend wait)",
    )
    parser.add_argument(
        "--team-prompt",
        action="store_true",
...
def main() -> int:
    ...
    if args.dispatch_ccb:
        ...
        existing = get_task_lock(task_locks, task_id)
        if existing and existing.get("runner") not in {"", "claude"}:
            print(
                f"[LOCKED] task {task_id} is locked by runner '{existing.get('runner')}'. "
                "Switch runner or unlock first."
            )
            return 2
        ...
        if not existing:
            upsert_task_lock(
                ...
                owner="team-lead",
                state="assigned",
                note="auto-lock on ccb dispatch",
            )
    ...
    if args.poll_ccb:
        ...
        if response:
            ...
            print(response)
            # TODO: Parse response format and update task lock automatically.
        else:
