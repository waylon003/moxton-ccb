...
- ✅ 标准化响应: `POST /orders/checkout` 与管理端订单接口使用 `OrderTransformer` 统一响应格式；用户/游客订单详情当前仍返回原始订单记录并附带 `addresses[]`
...
## 标准化订单响应（适用于 checkout / 管理端）

以下 `OrderResponseDTO` 仅适用于已接入 `OrderTransformer` 的端点：

- `POST /orders/checkout`
- `GET /orders/admin`
- `GET /orders/admin/:id`

`GET /orders/:id` 与 `GET /orders/guest/orders/:id` 当前仍需参考下方“当前用户/游客订单详情返回”章节。
...
## 当前用户/游客订单详情返回（2026-03-24 复核）

`GET /orders/:id` 与 `GET /orders/guest/orders/:id` 当前直接返回订单主记录与关联项，尚未经过 `OrderTransformer` 标准化。消费端需要特别注意：

- `data.address` 为完整地址字符串
- `data.addresses[]` 才是结构化地址数组，`addresses[0].fullAddress` 可作为结构化地址展示来源
- `data.items[].price` 为字符串单价，接口当前不返回 `unitPrice` / `subtotal`
- 登录用户订单通常带 `data.user`，游客订单则无该字段

```typescript
interface CurrentOrderDetailResponse {
  id: string
  orderNo: string
  userId: string | null
  totalAmount: string
...
**响应**: 当前返回 `CurrentOrderDetailResponse`
...
```json
{
  ...
  "message": "Success",
  "data": {
    "id": "cmmiuv7ra0000vfioi1b42ppu",
    "orderNo": "ORD17730410575418089",
    "userId": "cmm38r5q40000vf4k0192hb9p",
    "totalAmount": "747.98",
    "status": "PENDING",
    "orderType": "ONLINE",
    "guestName": "markTest",
    "guestEmail": "123456789@qq.com",
    "guestPhone": "0412345678",
    "guestCompany": "moxtontest",
    "paymentStatus": "PAYMENT_INITIATED",
    "paymentAttempts": 1,
    "lastPaymentAttemptAt": "2026-03-09T07:24:28.724Z",
    "stripePaymentIntentId": "pi_3T8yAxAdUxdJL62W1Ecac0VZ",
    "paymentId": "cmmiuve1m0008vfio3yidfe0w",
    "remarks": "test",
    "consignee": "markTest",
    "phone": "0412345678",
    "address": "Lalaguli Drive, Toormina New South Wales 2452, Australia",
    "metadata": "{\"guestId\":null,\"createdAt\":\"2026-03-09T07:24:17.541Z\",\"paymentExpiresAt\":\"2026-03-10T07:24:17.541Z\"}",
    "createdAt": "2026-03-09T07:24:17.542Z",
    "updatedAt": "2026-03-09T07:24:28.725Z",
    ...
    "items": [
      {
        "id": "cmmiuv7ra0002vfio4uzwzeak",
        "orderId": "cmmiuv7ra0000vfioi1b42ppu",
        "productId": "cmiscixh70000vf54v24qrz75",
        "quantity": 3,
        "price": "34",
        "createdAt": "2026-03-09T07:24:17.542Z",
        "product": {
          "id": "cmiscixh70000vf54v24qrz75",
          "name": "ikun",
          "price": "34",
          "images": [
            "https://moxton-image.oss-ap-southeast-1.aliyuncs.com/FLQ/products/1764907015759_13580056.jpg"
          ]
        }
      }
    ],
    "addresses": [
      {
        ...
        "district": null,
        "placeId": null,
        "createdAt": "2026-03-09T07:24:19.936Z"
      }
    ]
  },
  "timestamp": "2026-03-09T10:58:19.289Z",
  "success": true
}
```
...
**响应**: 成功时返回与 `GET /orders/:id` 相同的 `CurrentOrderDetailResponse` 结构，但游客订单通常满足以下特征：

- `userId = null`
- 不包含 `user` 字段
- `metadata` 字符串中保留 `guestId`、支付过期时间等上下文

**常见状态码**:

| HTTP 状态码 | 场景 | 说明 |
|------------|------|------|
| `200` | 获取成功 | 返回原始游客订单详情结构 |
| `400` | 缺少 `X-Guest-ID` | 返回 `X-Guest-ID header is required for guest orders` |
| `403` | 越权访问 | 订单不属于当前游客会话，或订单并非游客订单 |
| `404` | 订单不存在 | `id` 无对应订单 |
| `500` | 查询失败 | 当前控制器显式返回 `Failed to retrieve order` |

**错误响应示例（400）**:
```json
{
  "code": 400,
  "message": "X-Guest-ID header is required for guest orders",
  "timestamp": "2026-03-24T00:00:00.000Z",
  "success": false
}
```
...
4. 若支付查询因网络异常或 `5xx` 失败，可降级调用 `POST /payments/stripe/create-intent`，避免支付页初始化中断
5. 若创建接口仍返回 `400 Payment already in progress`，前端应立即再次调用 `GET /payments/order/:orderId`，若已生成 `activePayment` 则直接复用
6. 依据 `BACKEND-014` QA，历史已过期支付不会再阻塞重新创建 intent；重新创建后，旧记录会在支付列表中显示为 `CANCELLED`
...
**2026-03-24**:
- ✅ 依据 `SHOP-FE-013` QA，补充支付查询失败时允许降级创建 intent 的前端接入说明
- ✅ 补充 `400 Payment already in progress` 场景下“重查 `activePayment` 后复用”的闭环
