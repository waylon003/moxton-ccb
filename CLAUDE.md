...
- **唤醒开关**：Team Lead WezTerm 通知默认开启，可设置 `CCB_ENABLE_WEZTERM_NOTIFY=0` 关闭；Claude Code UI/手机端不依赖 WezTerm 通知。
...
| 查看状态 | `... -Action status` |
| 查看审批详情（文件型） | `... -Action show-approval -RequestId <ID>` |
| 恢复操作 | `... -Action recover -RecoverAction <reap-stale\|restart-worker\|reset-task\|normalize-locks\|prune-orphan-locks\|baseline-clean\|full-clean>` |
| 补建任务锁 | `... -Action add-lock -TaskId <ID>` |
| 批准审批请求（文件型） | `... -Action approve-request -RequestId <ID>` |
| 拒绝审批请求（文件型） | `... -Action deny-request -RequestId <ID>` |
| 批准本地 pane 审批 | `... -Action approve-local -WorkerName <WORKER> [-PromptType auto\|command_approval\|edit_confirm\|menu_approval]` |
| 放宽到本会话授权（编号菜单） | `... -Action approve-local-session -WorkerName <WORKER> [-PromptType auto\|menu_approval]` |
| 拒绝本地 pane 审批 | `... -Action deny-local -WorkerName <WORKER> [-PromptType auto\|command_approval\|edit_confirm\|menu_approval]` |
...
- 引擎默认取 `worker-map.json`；当前 `SHOP-FE / ADMIN-FE / BACKEND` 默认都走 `codex`。特殊场景仍可用 `-DispatchEngine codex|gemini` 临时覆盖。
- `baseline-clean` 为手动恢复动作，不会在每次派遣前自动执行。
- `prune-orphan-locks` 用于清理“任务文件在 active/completed 都不存在”的孤立锁；不要用临时脚本直接改 `TASK-LOCKS.json`。
...
- `route-monitor` 在处理每条 `report_route` 后只写 `config/teamlead-alerts.jsonl`；由 `notify-sentinel` 负责把新增回调转发给 Team Lead。
- 收到 `[APPROVAL-NEEDED]` 时先判断类型：文件型审批走 `show-approval -> approve-request/deny-request`；本地 pane 审批走 `approve-local/approve-local-session/deny-local`。
- `WORKER-SAMPLE` 不再注入 Team Lead pane，只落到 `config/worker-samples.log`；路由摘要落到 `config/teamlead-alerts.jsonl`。
...
- 规划阶段必须使用 `planning-gate`：
  ...
  - 若当前没有 active 任务，而用户输入的是问题描述/截图/缺陷列表/需求说明，默认进入 `planning-gate`，不得直接进入 `dispatch` / `dispatch-qa` / `recover`。
  - `assign_task.py` 不作为默认规划入口；仅允许在完成至少一轮澄清后，或用户明确要求诊断时，使用只读参数（如 `--list`、`--scan`）。
...
（当任务设置 `QA_LEVEL: skip` 时，dev success 将直接进入 `qa_passed`）
                ↓                      ↓
              blocked ←──────── requeue ────────
```
...
11. **Worker 禁止“先提问再等待确认”**；需要决策时必须 `report_route(status=blocked, ...)`
   - QA/DEV Worker 不得调用 `teamlead-control.ps1` 替 Team Lead 编排状态
...
16. **涉及登录/权限/真实数据流的 dev 与 QA 自测**：统一先读取 `05-verification/QA-IDENTITY-POOL.md`，优先使用固定测试凭据；固定凭据失败后才回退候选账号，禁止默认注册新账号探路
...
| Worker 类型 | 审批模式 | 说明 |
|-------------|---------|------|
| Dev (`*-dev`) | `-a never` | 完全无审批弹窗 |
| QA (`*-qa`) | `-a never` | 完全无审批弹窗 |
| Committer (`*-committer`) | `-a never` | 完全无审批弹窗 |
| 前端 (`shop-fe-*`/`admin-fe-*`) | 额外 `--enable js_repl` | 支持实时调试前端页面 |

Codex 沙箱策略：
- `*-dev`：`--sandbox danger-full-access`
- `*-qa`：`--sandbox danger-full-access`
- `*-committer`：`--sandbox danger-full-access`
dispatch 指令禁止 Codex 使用子代理（sub-agent），避免 pane 交互卡死。

### Codex 受控更新（Windows 场景）

Codex 在 Windows 上可能在 worker 启动时尝试自更新，并因 `codex.exe` 被占用而报 `EBUSY`。当前链路改为：

- `bootstrap` / 启动 Codex worker 前，由 Team Lead 控制层先执行一次受控更新
- worker 默认直接运行 native `codex.exe`，避免启动时再次触发 npm 自更新

可选：手动指定 native 二进制路径：

```bash
set CODEX_NATIVE_BIN=C:\path\to\codex.exe
```

说明：
- 如需强制退回旧的 npm wrapper，可设置：`set CODEX_USE_WRAPPER=1`
- 若存在正在运行的 `codex.exe` 进程，受控更新会跳过，并在下次干净启动时重试。
...
### 审批转发闭环（Agent Teams + Per-Worker Watcher）

当前审批链路拆成两层：
- `scripts/pane-approval-watcher.ps1`：由 `dispatch/dispatch-qa` 自动拉起，每个活跃 worker 独享一个 watcher，只负责扫描自己的 pane、本地低风险最小按键、把高风险/未知审批写入 `config/local-approval-events.jsonl` / `config/local-approval-state.json` / `config/teamlead-alerts.jsonl`。
- `notify-sentinel`：只消费 `approval-requests.json`、`config/teamlead-alerts.jsonl`、`config/local-approval-events.jsonl`，把需要 Team Lead 处理的审批/回调转发出来。
...
- teamlead-control -Action status 只裁剪历史 resolved；pending 不会自动超时拒绝。
- 如需强制清理，使用 recover -RecoverAction baseline-clean。
- 若需要超时自动拒绝，由 notify-sentinel 按 APPROVAL_REQUEST_TTL_SECONDS 处理。

## 后台回调监听

dispatch 后由控制器自动确保后台能力：
- route-monitor.ps1（常驻，主链路）
- pane-approval-watcher.ps1（每个活跃 worker 一个，负责本地 pane 审批捕获）
- notify-sentinel（Agent Teams 通知队友：只消费事件并转发审批 + 回调提醒）

Agent Teams 启用（实验特性）：
- 默认启用严格门槛：派遣前必须创建 notify-sentinel；如需关闭，设置 `TEAMLEAD_STRICT_NOTIFY_SENTINEL=0`。
- 若设置 `CCB_ROUTE_MONITOR_NOTIFY=0`，route-monitor 将不再唤醒 Team Lead，必须依赖 notify-sentinel 转发提醒。
- 若 notify-sentinel ready 标记在有效期内，route-monitor 仅写 `teamlead-alerts.jsonl`，不再 send-text 刷屏。
- 在 settings.json 或环境变量中设置 CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1
- 推荐 teammateMode: in-process，也可使用 claude --teammate-mode in-process
- 在 Team Lead 会话中创建团队并添加 notify-sentinel，指令其阅读 E:\\moxton-ccb\\.claude\\agents\\notify-sentinel.md
- **必须继续发送启动指令使其进入轮询**：`@notify-sentinel 读取 E:\\moxton-ccb\\.claude\\agents\\notify-sentinel.md 并立刻进入循环；只要检测到新增 route/approval/local approval 事件，第一动作必须先给 Team Lead 发一条 teammate 消息，再继续轮询；禁止只更新 ready.json 或只在你自己的窗口总结。`
- notify-sentinel 启动后必须输出一次 `[WATCH-READY]` 并写入 `config/notify-sentinel.ready.json`
- 若未写入，执行 `powershell -NoProfile -ExecutionPolicy Bypass -File "E:\moxton-ccb\scripts\teamlead-control.ps1" -Action notify-ready` 补写；但 `notify-ready` 只补 ready 标记，不能替代 notify-sentinel 真正进入轮询
